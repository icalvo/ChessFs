...
module Option =
    ...
    let mapList fn opt =
        ...
        |> Option.map fn 
        |> Option.defaultValue []
    let toResult errorData = function
        | Some x -> Ok x
...
module Result =
    ...
    let filter: bool -> 'e -> Result<'a, 'e> -> Result<'a, 'e> = fun b e ->
        function
        | Ok x -> if b then Ok x else Error e
        | Error e -> Error e 
    ...
    let failed = function | Ok _ -> false | _ -> true
    let succeeded = function | Ok _ -> true | _ -> false
...
module Seq =
    ...
    /// Therefore, the generator function has the simpler signature:
    ///     'State -> 'State option
    /// Instead of:
    ///     'State -> ('T * 'State) option
    /// </remarks>
    let unfoldSimple: ('State -> 'State option) -> 'State -> seq<'State> =
        fun nextFn -> nextFn >> Option.map (fun x -> (x, x)) |> Seq.unfold
