...
let stateActionsFailingStateMachine initialState actions translateAction normalizeInput execute input =
    let transition sa i =
        let availableActions = sa |> actions
        let action =
            availableActions
            |> List.tryFind (fun x -> (x |> translateAction) = (i |> normalizeInput))
        match action with
...
let stateActionsIgnoringStateMachine initialState actions translateAction normalizeInput execute input =
    let transition sa i =
